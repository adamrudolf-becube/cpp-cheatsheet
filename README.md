...
## Why do I need `virtual` and pointers to achieve runtime polymorphism?
...
Something called a VTABLE is created, that is a static array of function poiners. This VTABLE is a static data member of the class. This contains function pointers, the addresses of all virtual functions in the class. A poiner VPTR is then generated for each object instance. These can be maintained runtime to find the actual function to call. So what function body to execute when a function is called is now known at compile time, the code is actually looking up function pointers from an array and calls them, kind of like callbacks. Again, this is written in simple C++ code, but the compiler generates that code for you. To generate this mechanism is what you ask from the compiler when you write `virtual`.

(*Note*: in Java vitual is the default behaviour, and you need to specify explicitly if you *don't* want a function to be virtual. In my opinion from the usage perspective this is logical, beause if you override a function, you do this to achieve polymorphism, and it's very rare that you override a function and you don't want a polymorphic behaviour. On the other hand C++ logic reflects 2 things: 1) you need to be explicit if you want to add extra mechanisms, and C++ only does that if you ask it to, and 2) you *have to* mark functions that are mean to be overridden. You can make sure to retain the functionality if an accidental override happens by someone else which is not meant to.)
